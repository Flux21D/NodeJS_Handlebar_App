...
gulp.task('thumbimage', () => {
  return gulp.src('./src/public/*.png')
        .pipe(imagemin())
...
gulp.task('pdf-vendor', (cb) => {
  pump([
    gulp.src('./src/public/pdf/**/*'),
    gulp.dest('./lib/public/pdf'),
  ], cb);
});
...
gulp.task('watch', () => {
  ...
  gulp.watch(['./src/public/*.png'], ['thumbimage']);
  ...
  gulp.watch(['./src/public/css/*.css'], ['minify-css-custom']);
  gulp.watch(['./src/public/fonts/*'], ['font-vendor']);
  gulp.watch(['./src/public/pdf/*'], ['pdf-vendor']);
});
